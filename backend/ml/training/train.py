...
def main() -> None:
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if config.save_every_epoch:
            epoch_path = config.model_dir / f"{config.model_name}_epoch_{epoch}.pth"
            save_checkpoint(model, epoch_path)

        if val_metrics["top1"] > best_val_top1:
            best_val_top1 = val_metrics["top1"]
            best_path = config.model_dir / f"{config.model_name}_best.pth"
    ...
    history_path = config.model_dir / f"{config.model_name}_history.json"
