...
def main() -> None:
    ...
    expanded_review_path = cleaning_dir / "expanded_outlier_review" / "expanded_outlier_review_candidates.csv"

    if not main_review_path.exists():
        raise FileNotFoundError(f"Missing main review CSV: {main_review_path}")
    if not expanded_review_path.exists():
        raise FileNotFoundError(f"Missing expanded outlier review CSV: {expanded_review_path}")

    main_rows = read_csv(main_review_path)
    expanded_rows = read_csv(expanded_review_path)
    ...
    for row in expanded_rows:
        ...
        if existing_decision == "" and new_decision in {"keep", "remove", "uncertain"}:
            ...
            existing["review_notes"] = " | ".join([x for x in [note_old, note_new] if x]).strip(" |")
            updates += 1
        else:
            note_old = existing.get("review_notes", "") or ""
            note_new = row.get("review_notes", "") or ""
    ...
    print("Merged expanded outlier review into main review CSV.\n")
