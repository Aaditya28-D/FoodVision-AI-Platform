...
def build_contact_sheet(class_name: str, rows: list[dict], dataset_root: Path, output_path: Path) -> None:
    ...
    title = f"{class_name} — expanded outlier review ({total} shown)"
...
def main() -> None:
    ...
    output_dir = project_root / "backend" / "artifacts" / "dataset_cleaning" / "expanded_outlier_review"
    ...
    review_csv_path = output_dir / "expanded_outlier_review_candidates.csv"
    ...
    for class_name, class_rows in sorted(rows_by_class.items()):
        sheet_path = output_dir / f"{class_name}_expanded_outliers.jpg"
