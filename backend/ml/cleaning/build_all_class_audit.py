...
def build_rows_for_class(
    ...
    for row in outlier_rows:
        ...
        used.add(image_path)
        added_outliers += 1
        if added_outliers >= NUM_OUTLIERS:
            break
...
def build_contact_sheet(class_name: str, rows: list[dict], dataset_root: Path, output_path: Path) -> None:
    ...
    title = f"{class_name} — all-class audit"
...
def main() -> None:
    ...
    output_dir = cleaning_dir / "all_class_audit"
    audit_csv_path = output_dir / "all_class_audit_candidates.csv"
    ...
    for class_name in class_names:
        ...
        sheet_path = output_dir / f"{class_name}_audit.jpg"
    ...
    print(f"Total classes covered: {len(class_names)}")
