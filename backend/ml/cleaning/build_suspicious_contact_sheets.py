...
def load_suspicious_rows(csv_path: Path) -> dict[str, list[dict]]:
...
def build_contact_sheet(
    ...
    title = f"{class_name} — suspicious samples ({total} shown)"
    ...
    for idx, row in enumerate(rows):
        ...
        flags = row["flags"]
        text_1 = shorten_text(filename, 26)
        text_2 = shorten_text(flags, 28)
...
def main() -> None:
    ...
    suspicious_csv = project_root / "backend" / "artifacts" / "dataset_cleaning" / "suspicious_images.csv"
    output_dir = project_root / "backend" / "artifacts" / "dataset_cleaning" / "contact_sheets"
    ...
    rows_by_class = load_suspicious_rows(suspicious_csv)
    if not rows_by_class:
        raise RuntimeError("No suspicious rows found in suspicious_images.csv")

    for class_name, rows in sorted(rows_by_class.items()):
        output_path = output_dir / f"{class_name}_contact_sheet.jpg"
        build_contact_sheet(
    ...
    print("\nDone.")
    print(f"Contact sheets saved under: {output_dir}")
