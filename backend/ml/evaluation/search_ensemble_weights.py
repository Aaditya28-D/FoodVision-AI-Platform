...
def main() -> None:
    parser = argparse.ArgumentParser(description="Search weighted ensemble for EfficientNet + MobileNet.")
    ...
    output_path = project_root / "backend" / "models" / "reports" / "weight_search_effnet_mobilenet.json"
    ...
    mob_model = loader.load_model(ModelName.MOBILENET_V3_LARGE)
    ...
    for eff_weight in weights:
        mob_weight = round(1.0 - eff_weight, 1)
        correct = 0
        total = 0

        print(f"\nTesting weights: eff={eff_weight}, mob={mob_weight}")
    ...
    report = {
        "search_name": "effnet_mobilenet_weight_search",
